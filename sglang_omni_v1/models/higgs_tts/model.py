...
:func:`sglang_omni_v1.models.higgs_tts.bootstrap.register_higgs_tts_in_sglang`).
"""
...
from sglang_omni_v1.models.higgs_tts.modeling import (
    HiggsFusedMultiTextEmbedding,
    HiggsFusedMultiTextHead,
)
from sglang_omni_v1.models.higgs_tts.sampler import STOP_CODE, HiggsSamplerState
from sglang_omni_v1.models.higgs_tts.sampler import step as sampler_step
from sglang_omni_v1.models.higgs_tts.weight_loader import DiscreteWeightMapper
...
class HiggsTTSModel(nn.Module):
    ...
    def decode_codebooks_batch(
        ...
        This is the core "forward-embedded" hook called at the tail of
        :meth:`forward` (mirrors s2_pro's ``_decode_codebooks``). For
        each row ``b``:
    ...
    def forward(
        ...
        **kwargs,
        ...
        """Forward-embedded multi-codebook decode (mirrors s2_pro's
        `_decode_codebooks` pattern).
        ...
        — matches boson-vllm's ``embed_input_ids`` line 1103-1129.
        ...
        # Prune to last-token positions for prefill (same as s2_pro:
        # `fishaudio_s2_pro/sglang_model.py:370-372`).
        if (
