...
"""Multi-codebook sampler state machine for Higgs TTS (PR4a).

Ported from boson-vllm ``sample_with_multimodal_metadata`` in
``vllm/model_executor/models/higgs_multimodal_qwen3.py:1239-1350``. This module
is pure torch / pure Python — no sglang dependency — so it can be exercised
in isolation. PR4b wires it into sglang's per-step forward loop.
...
from sglang_omni_v1.models.higgs_tts.delay_pattern import BOC_ID, EOC_ID
