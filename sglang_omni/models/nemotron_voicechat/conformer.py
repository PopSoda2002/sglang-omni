...
class StreamingPerception:
    ...
    def __init__(self, perception: AudioPerception) -> None:
        ...
        self.xscale = math.sqrt(encoder.layers[0].norm_out.weight.shape[0]) if encoder.xscaling else 1.0
        ...
        with torch.inference_mode():
            offsets_P = torch.arange(self.max_keys - 1, -1, -1, device=self.device, dtype=torch.float32)
            angles_PH = einsum(offsets_P, encoder.pos_enc.inverse_freq_H, "p, h -> p h")
            encoding_PD = rearrange(
                torch.stack((angles_PH.sin(), angles_PH.cos()), dim=-1), "p h two -> p (h two)"
            ).to(self.dtype)
            ...
            self.posproj_LPHS = [
                rearrange(layer.self_attn.linear_pos(encoding_PD), "p (h s) -> p h s", h=self.num_heads)
    ...
    def reset(self) -> None:
        ...
        self.sample_buffer = torch.zeros(preprocessor.left_padding, device=self.device, dtype=self.dtype)
        ...
        self.sub_caches = [torch.zeros(shape, device=self.device, dtype=self.dtype) for shape in sub_cache_shapes]

        layers = perception.encoder.layers
        self.key_caches = [torch.zeros(0, self.num_heads, self.head_size, device=self.device, dtype=self.dtype) for _ in layers]
        self.value_caches = [torch.zeros(0, self.num_heads, self.head_size, device=self.device, dtype=self.dtype) for _ in layers]
        self.conv_caches = [
            torch.zeros(1, layer.conv.depthwise_conv.in_channels, layer.conv.depthwise_conv.left_padding,
    ...
    def _encode(self, mel_1TM: torch.Tensor) -> torch.Tensor:
        ...
        for index, layer in enumerate(self.perception.encoder.layers):
            hidden_11D = hidden_11D + FEED_FORWARD_RESIDUAL_SCALE * layer.feed_forward1(layer.norm_feed_forward1(hidden_11D))
            hidden_11D = hidden_11D + self._attend(index, layer.self_attn, layer.norm_self_att(hidden_11D))
            hidden_11D = hidden_11D + self._convolve(index, layer.conv, layer.norm_conv(hidden_11D))
            hidden_11D = hidden_11D + FEED_FORWARD_RESIDUAL_SCALE * layer.feed_forward2(layer.norm_feed_forward2(hidden_11D))
    ...
    def _subsample(self, mel_1TM: torch.Tensor) -> torch.Tensor:
        ...
        return self.perception.encoder.pre_encode.out(rearrange(hidden_1CTM, "b c t m -> b t (c m)"))

    def _attend(self, index: int, attention, hidden_11D: torch.Tensor) -> torch.Tensor:
        query_HS = rearrange(attention.linear_q(hidden_11D), "1 1 (h s) -> h s", h=self.num_heads)
        key_HS = rearrange(attention.linear_k(hidden_11D), "1 1 (h s) -> h s", h=self.num_heads)
        value_HS = rearrange(attention.linear_v(hidden_11D), "1 1 (h s) -> h s", h=self.num_heads)
        keys_KHS = torch.cat((self.key_caches[index], key_HS[None]))[-self.max_keys:]
        values_KHS = torch.cat((self.value_caches[index], value_HS[None]))[-self.max_keys:]
        self.key_caches[index] = keys_KHS
        self.value_caches[index] = values_KHS

        num_keys = keys_KHS.shape[0]
        content_KH = einsum(query_HS + attention.pos_bias_u, keys_KHS, "h s, k h s -> k h")
        position_KH = einsum(
            query_HS + attention.pos_bias_v, self.posproj_LPHS[index][self.max_keys - num_keys:], "h s, k h s -> k h"
        )
        weights_KH = ((content_KH + position_KH) / math.sqrt(self.head_size)).softmax(dim=0)
