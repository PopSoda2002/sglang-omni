...
def create_perception_executor(model_path: str, *, dtype = None, device = None):
    ...
    def encode(payload: StagePayload) -> StagePayload:
        ...
        samples = waveform_1S.shape[-1]
        assert samples % SAMPLES_PER_FRAME == 0, (
            f"VoiceChat consumes whole 80 ms frames; got {samples} samples, "
            f"which is not a multiple of {SAMPLES_PER_FRAME}."
        )

        frames = module(waveform_1S.to(device=device, dtype=parameter_dtype))

        expected = samples // SAMPLES_PER_FRAME
        assert frames.shape[1] == expected, (
            f"Perception returned {frames.shape[1]} frames for {expected} frames "
            "of audio; the 12.5 Hz timeline has drifted."
        )

        state.acoustic_frames = frames[0]
        state.num_frames = expected
