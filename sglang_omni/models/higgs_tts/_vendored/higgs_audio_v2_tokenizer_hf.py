...
Source: ``higgs-mm/higgs_mm/nn/encoders/higgs_audio_v2_tokenizer_hf.py``
(upstream reference: https://github.com/huggingface/transformers/pull/40294).

Combined two upstream files:
- ``configuration_higgs_audio_v2_tokenizer.py``
- ``modeling_higgs_audio_v2_tokenizer.py``

Why vendored: the upstream higgs-mm copy depends on ``transformers>=5``
(``from transformers import initialization``), but sglang-omni pins
``transformers<5``. This file is a near-verbatim copy with a minimal
compatibility shim so the same checkpoint (``bosonai/higgs-audio-v2-
tokenizer``) loads cleanly under either transformers major version.

Patches applied vs. upstream:
1. Replaced ``from transformers import initialization as init`` with a
   ``torch.nn.init``-backed shim that also provides ``init.copy_``
   (upstream's single non-``torch.nn.init`` call).
"""
